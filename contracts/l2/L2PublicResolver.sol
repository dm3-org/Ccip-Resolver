...
contract L2PublicResolver is PublicResolver(ENS(0)) {
    ...
    function setText(bytes32 node, string calldata key, string calldata value) external {
        bytes32 ownedNode = keccak256(abi.encode(node, msg.sender));
        texts[ownedNode][key] = value;
        emit TextChanged(ownedNode, key, key);
    }
    ...
    function setAddr(bytes32 node, uint coinType, bytes memory a) public {
        bytes32 ownedNode = keccak256(abi.encode(node, msg.sender));
        emit AddressChanged(ownedNode, coinType, a);
        if(coinType == COIN_TYPE_ETH) {
            emit AddrChanged(ownedNode, bytesToAddress(a));
        }
        _addresses[ownedNode][coinType] = a;
    }
